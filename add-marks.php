...
<head>
    <meta charset="UTF-8">
    <title>Add Marks - Study-Sphere</title>
...
<body>
    ...
    <div class="form-container">
        <h2>ADD MARKS</h2>
        <form action="add-marks.php" method="post" autocomplete="off">
            <input type="text" name="name" placeholder="Student Name" required>
            <input type="text" name="branch" placeholder="Branch" required>
            <input type="number" name="semester" placeholder="Semester" required min="1">
            <input type="text" name="roll" placeholder="Roll Number" required>
            <div style="width:100%;margin-bottom:6px;">
                <strong>Subjects and Marks</strong>
            </div>
            <?php for($i=1;$i<=5;$i++): ?>
            <div class="subject-row">
                <input type="text" name="subject<?php echo $i; ?>" placeholder="Subject <?php echo $i; ?> Name" required>
                <input type="number" name="marks<?php echo $i; ?>" placeholder="Marks" required min="0">
            </div>
            <?php endfor; ?>
            <input type="submit" value="Save Marks" class="sub">
        </form>
        <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $conn = new mysqli('mysql.railway.internal', 'root', 'GJifTuTKslzyFAUQochWGXciqLxvOpEU', 'railway');
            ...
            // Prepare for 5 subjects and marks
            $subjects = [];
            $marks = [];
            for($i=1;$i<=5;$i++) {
                $subjects[] = $conn->real_escape_string(trim($_POST["subject$i"]));
                $marks[] = (int)$_POST["marks$i"];
            }
            ...
            $conn->query("CREATE TABLE IF NOT EXISTS marks (
                ...
                semester INT,
                roll VARCHAR(30),
                subject1 VARCHAR(100), marks1 INT,
                subject2 VARCHAR(100), marks2 INT,
                subject3 VARCHAR(100), marks3 INT,
                subject4 VARCHAR(100), marks4 INT,
                subject5 VARCHAR(100), marks5 INT
            )");
            $sql = "INSERT INTO marks 
                (name, branch, semester, roll, 
                 subject1, marks1, subject2, marks2, subject3, marks3, subject4, marks4, subject5, marks5)
                VALUES 
                ('$name', '$branch', $semester, '$roll', 
                 '{$subjects[0]}', {$marks[0]}, '{$subjects[1]}', {$marks[1]}, '{$subjects[2]}', {$marks[2]}, '{$subjects[3]}', {$marks[3]}, '{$subjects[4]}', {$marks[4]})";
            if ($conn->query($sql) === TRUE) {
                echo "<div style='color:green;margin-top:10px;'>Marks Saved Successfully!</div>";
            } else {
